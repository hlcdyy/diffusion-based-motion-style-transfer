...
def get_opt(opt_path, device):
    ...
    if opt.dataset_name == 't2m':
        opt.data_root = './processed_data/HumanML3D' # use our revised representation
        ...
        opt.data_root = './processed_data/KIT-ML'  
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.text_dir = pjoin(opt.data_root, 'texts')
        opt.joints_num = 21
        opt.dim_pose = 251
        ...
        if opt.dataset_name == 'bandai-1_posrot':
            opt.data_root = './processed_data/bandai-1/'
        else:
            opt.data_root = './processed_data/bandai-2/'  # for new model save_bandai_train_newtpose
        opt.t2m_root = './processed_data/bandai-2/'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.joints_num = 21
        opt.dim_pose = 190
        opt.max_motion_length = 196
        ...
        opt.data_root = './processed_data/style_xia/'
        opt.t2m_root = './processed_data/style_xia/'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.joints_num = 20
        opt.dim_pose = 181
