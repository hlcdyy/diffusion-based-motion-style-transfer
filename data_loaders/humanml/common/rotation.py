...
def q2eul(q, order='xyz'):
    """
    the output euler rotation order is in fact zyx when "order = 'xyz'",
    but the value is 'xyz' order 
    """
    q = qnorm(q)
    q0 = q[..., 0]
    q1 = q[..., 1]
...
def q2eul_new(q, order='xyz'):
    """
    The ouput value order format is equal to the real rotation order 
    """
    q = qnorm(q)
    q0 = q[..., 0]
    q1 = q[..., 1]
...
def rotm2cont6d(rotm):
    ...
    return torch.cat([rep[..., 0], rep[..., 1]], dim=-1)
...
def rotm2cont6d(rotm):
    cont_6d = torch.cat([rotm[..., 0], rotm[..., 1]], dim=-1)
    return cont_6d
...
if __name__ == "__main__":
    ...
    eul2 = torch.Tensor([[np.pi/6, np.pi/7, np.pi/8]])
    quats2 = eul2q(eul2, 'zyx')
    mat2 = q2rotm(quats2)
    print(q2rotm(qmultipy(quats, quats2)))
    print(np.dot(mat, mat2))

    v1 = torch.Tensor(np.array([0, 0, 1], dtype=np.float32))
    v2 = torch.Tensor(np.array([0, 1, 0], dtype=np.float32))
    q_b = quatbetween(v1, v2)
    print(quatbetween(v1, v2))
    print(qrot(q_b, v1))
    print(np.dot(q2rotm(q_b), v1.T))
